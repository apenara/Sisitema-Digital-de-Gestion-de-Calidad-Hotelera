...
const CompanyManagement: React.FC = () => {
  ...
  const filteredCompanies = companies.filter(company => {
    ...
    const matchesOrg = !filters.organizationId || company.organizationId === filters.organizationId;
    const matchesType = !filters.type || company.type === filters.type;
    const matchesStatus = !filters.status || company.status === filters.status;
    const matchesIndustry = !filters.industryType || company.industry.type === filters.industryType;
    
    return matchesSearch && matchesOrg && matchesType && matchesStatus && matchesIndustry;
  });
  ...
  const handleCreateCompany = async () => {
    try {
      const createData: CreateCompanyInput = {
        ...
        settings: {
          departments: [], // Se pueden agregar departamentos por defecto según la industria
          language: 'es',
  ...
  const handleDeleteCompany = async () => {
    ...
    try {
      // TODO: Implementar deleteCompany en el slice
      console.log('Delete company functionality not implemented yet');
  ...
  const getTypeBadge = (type: string) => {
    ...
    return (
      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${typeClasses[type as keyof typeof typeClasses] || typeClasses.independent}`}>
        {typeLabels[type as keyof typeof typeLabels] || type}
      </span>
    );
  };

  const getIndustryBadge = (industryType: IndustryType) => {
    const color = getIndustryColor(industryType);
    const label = getIndustryLabel(industryType);
  ...
  const getSizeBadge = (size: string) => {
    ...
    return (
      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${sizeClasses[size as keyof typeof sizeClasses] || sizeClasses.medium}`}>
          ...
          <div className="py-6 md:flex md:items-center md:justify-between">
            <div className="flex-1 min-w-0">
              ...
              <p className="mt-1 text-sm text-gray-500">
                Administrar empresas de toda la plataforma
              </p>
            </div>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-5">
            ...
            <div>
              ...
              >
                ...
                <option value="pending_setup">Configuración Pendiente</option>
              </select>
            </div>
            <div>
                      ...
                      <div className="flex-1">
                        <div className="flex items-center">
                          ...
                          <div className="ml-3 flex space-x-2">
                            ...
                            {getIndustryBadge(company.industry.type)}
                            {getSizeBadge(company.industry.size)}
                          </div>
                        </div>
                        ...
                        <div className="mt-2 flex items-center space-x-4 text-sm text-gray-500">
                          ...
                          <span>•</span>
                          <span>{company.industry.employeeCount || 'N/A'} empleados</span>
                          {company.organizationId && (
                            <>
                              <span>•</span>
                              <span>{organizations.find(org => org.id === company.organizationId)?.name}</span>
                            </>
                          )}
                          {company.industry.certifications && company.industry.certifications.length > 0 && (
                            <>
                              <span>•</span>
                              <span>{company.industry.certifications.join(', ')}</span>
                            </>
                          )}
                        </div>
                      </div>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => setFormData({
                        ...formData,
                        industry: { ...formData.industry, type: e.target.value as IndustryType }
                      })}
                      className="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                    >
                      {industries.map(industry => (
                  ...
                  <div>
                    ...
                    <select
                      value={formData.industry.size}
                      onChange={(e) => setFormData({
                        ...formData,
                        industry: { ...formData.industry, size: e.target.value as any }
                      })}
                      className="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                    >
                      ...
                      <option value="enterprise">Corporación</option>
                    </select>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700">Empleados</label>
                    <select
        ...
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
          <div className="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            <div className="mt-3 text-center">
              ...
              <h3 className="text-lg font-medium text-gray-900 mt-5">Eliminar Empresa</h3>
              <div className="mt-2 px-7 py-3">
                <p className="text-sm text-gray-500">
                  ¿Estás seguro de que quieres eliminar la empresa "{selectedCompany.name}"? 
